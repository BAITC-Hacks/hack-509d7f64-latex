...
- **migrations/ + mock_seed.go**: embedded SQL migrations, recorded in `router_schema_migrations` and applied by `OpenSQLite`. 001 is the schema. 002 seeds `mock_backend_state` (one JSON document) once from the embedded `mock_backend.json`, binding `:mock_backend`, `:mock_backend_sha256`, `:mock_sequence` and `:now`, so restarts never overwrite tool mutations. 003 adds the read-only views `mock_clients`, `mock_policies`, `mock_claims`, `mock_payments` and `mock_records`. `cmd/migrate` prints per-view counts; `-reset-data` re-seeds the backend, and `-reset-all` also wipes sessions and turns.
