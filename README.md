...
On startup the router opens `DB_PATH`, applies the embedded migrations
(`backend/internal/router/migrations/*.sql`) and seeds the synthetic business records from
`mock_backend.json` once. Later starts never overwrite mutated records. `go run ./cmd/migrate`
does the same without serving, and is optional. To reset all sessions and mock data, stop the
router and delete the database file and its `-wal`/`-shm` companions. Build a binary with
`go build -o voice-router ./cmd/router` from `backend/`.
