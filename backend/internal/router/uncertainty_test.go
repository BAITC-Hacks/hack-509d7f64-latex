...
func TestAssess(t *testing.T) {
	...
	cases := []struct {
		...
		{name: "continuation of active frame skips retrieval", d: cont(routed("SC01", .95)), s: active, shortlist: ranking("SC05", .8, "SC02", .6), path: "full",
			verdict: "execute", keys: []string{"model"}},
		...
		{name: "continuation of queued frame", d: cont(routed("SC33", .95)), s: &Session{Queue: []*Frame{{ScenarioID: "SC33"}}}, shortlist: ranking("SC05", .8, "SC06", .1), path: "full",
			verdict: "execute", keys: []string{"model"}},
		{name: "quote to purchase follows last completed", d: cont(routed("SC02", .95)), s: &Session{LastCompleted: &Frame{ScenarioID: "SC01"}}, shortlist: ranking("SC05", .8, "SC01", .6), path: "full",
			verdict: "execute", keys: []string{"model"}},
		...
		{name: "continuation ignores boundary neighbour in shortlist", d: cont(routed("SC01", .95)), s: active, shortlist: ranking("SC02", .8, "SC01", .2), path: "full",
			verdict: "execute", keys: []string{"model"}},
